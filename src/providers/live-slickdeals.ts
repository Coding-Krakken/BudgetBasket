...
function matchStore(
  ...
  return (
    stores.find((s) => norm.includes(s.name.toLowerCase())) ?? null
  );
}

export class LiveSlickdealsProvider extends BaseProvider {
  readonly id = "live-slickdeals";
  ...
  async fetchOpportunities(
    ...
    try {
      ...
      return this.failure(`Slickdeals RSS fetch failed: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
}

export const liveSlickdealsProvider = new LiveSlickdealsProvider();
